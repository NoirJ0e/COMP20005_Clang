...
void stage_1_result(weather_data_t *data, int line_count, max_data_t *max) {
    ...
    printf(
        "S1, row %5d: at %02d:%02d on %02d/%02d/%4d, solar = %4.0lf, wind = "
        "%5.2lf, temp = %4.1lf\n",
        data[0].position, data[0].hour, data[0].minute, data[0].day,
        data[0].month, data[0].year, data[0].solar, data[0].wind, data[0].temp);
    printf(
        "S1, row %5d: at %02d:%02d on %02d/%02d/%4d, solar = %4.0lf, wind = "
        "%5.2lf, temp = "
        "%4.1lf\n",
...
void stage_2_result(weather_data_t *data, int line_count) {
    ...
    printf(
        "S2,        Jan  Feb  Mar  Apr  May  Jun  Jul  Aug  Sep  Oct  Nov  "
        "Dec\n");
    printf(
        "S2,       "
        "+----+----+----+----+----+----+----+----+----+----+----+----+\n");
